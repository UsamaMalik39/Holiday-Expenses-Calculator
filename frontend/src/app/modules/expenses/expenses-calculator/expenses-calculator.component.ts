...
export class ExpensesCalculatorComponent implements OnInit {
  ...
  createItem(): any {
    return this._fb.group({
      name: [null,[Validators.required,Validators.pattern('^[_A-z]*((-|\s)*[_A-z])*$'),Validators.minLength(2),Validators.maxLength(20)]],
      amount: [null,[Validators.required,Validators.min(1),Validators.maxLength(5)]],
    });
  }
  ...
  onSubmit() {
    if(!this.formForExpenses.valid){
      ...
      this._calculatorSrv.getExpensesCalculation(this.expensesArray?.value).subscribe((data:any)=>{
        if(!data.error){
